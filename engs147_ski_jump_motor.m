...
%v3
% Gcz = zpk([0.5, 0.1], [0.15, 1], 1, dt);
% K = 0.14;
%v4
Gcz = zpk([0.4, 0.53], [0.35, 1], 1, dt);
K = 0.16;
%v5
% Gcz = zpk([0.3, 0.53], [0.4, 1], 1, dt);
% K = 0.26;
Hs = 1;
wheel_ref = 7.5; 
...
filepath = "C:\Users\noahd\OneDrive\Desktop\ENGS 147\Final Project\motor_char_v4.txt";
...
%for test_num = 6:8
    ...
    dt_ms = parse.test_info(7).value;
    time_to_step_ms = parse.test_info(8).value;
    step_duration_ms = parse.test_info(9).value;
    ...
    hold on;
    [sim_step, ~] = step(wheel_ref_arduino*new_disc_cl, (step_time(1):(dt_ms/1000):step_time(end)));
    stairs(step_time, sim_step);
    hold off;
